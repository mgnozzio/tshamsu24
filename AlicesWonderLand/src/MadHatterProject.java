...
private VisibleImage texting;


//private DrawingCanvas canvas;
//private Image[] dsp;

//another way for buffering
...
//	Image clicker = getImage("clicking.png");
//	texting = new VisibleImage(clicker, WIDTH/2, HEIGHT/2, 100, 100, canvas );
...
public static BufferedImage GetSpriteAtlas(String fileName, int x, int y, int w, int h) {
	BufferedImage img = null;
	InputStream is = MadHatterProject.class.getResourceAsStream( fileName);
	try {
		img = ImageIO.read(is);
...
public void onMouseClick(Location l) {
	...
	enemies = new RedArmy(invaders, false, this, canvas);
	...
	BufferedImage img = GetSpriteAtlas("threealices.png", 0, 0, 96, 32);
...
public void keyTyped(KeyEvent e) {
	if ( e.getKeyCode() == KeyEvent.VK_SPACE ||
		...
		alice.leftOn(true);
		Image aliceleft = getImage("alicetoleft.png");
		alice.UpdateAlice(aliceleft);


} 	else if ( e.getKeyCode() == KeyEvent.VK_RIGHT ) {

	alice.rightOn(true);
	Image aliceshoot = getImage("alicetoright.png");
	alice.UpdateAlice(aliceshoot);
