...
public class RedArmy extends ActiveObject implements Shootable{
	...
	private VisibleImage[][] RedArmy = new VisibleImage[6][9];
	
	public RedArmy(Image[] invaders, boolean fromalice, MadHatterProject main, DrawingCanvas canvas) {

		this.main = main;
		this.canvas=canvas;
		for (int row = 0; row < RedArmy.length; row++) {
			for (int col = 0; col < RedArmy[0].length; col++) {
				RedArmy[row][col] = new VisibleImage(invaders[row], 29+col*ENEMY_X, row*ENEMY_Y, 
						ENEMY_SIZE, ENEMY_SIZE, canvas);
			}
		}
		rows = new int[RedArmy.length][RedArmy[0].length]; 
	...
	public boolean tryShoot(FilledRect r) {
		for(int i = 0; i < RedArmy.length; i++) {
			for(int j = 0; j < RedArmy[0].length; j++) {
				if(RedArmy[i][j].overlaps(r)) {
					try {
					RedArmy[i][j].removeFromCanvas();
					r.removeFromCanvas();
					rows[i][j] = 1;
					return true;
					} catch (Exception e) {
	...
	public void clear() {
		...
		for (int i = 0; i < RedArmy.length; i++) {
			for (int j = 0; j < RedArmy[0].length; j++) {
				if(RedArmy[i][j].overlaps(main.b)) {
					RedArmy[i][j].removeFromCanvas();
				}
			}
		}
